...
class TFBase(object):
  ...
  def addDefaultParameters(self):
    ...
    self.parser.add_argument('--save_dir', type=str, default='SAVE_DIR',
                       help='save directory')
  ...
  def loadData(self):
    ...
    # from pickle load the training data
    f = open(data_file,"rb")
    data = pickle.load(f)

    # data["input"] shape : (971) -> each array shape (*,28) ; data["output"] shape : (971) -> each array shape (*,20)  "validation" is empty 0
    inps = {"training": data["input"], "validation": data["vinput"]} 
    outps = {"training": data["output"], "validation": data["voutput"]} 
    f.close()

    self.dimin = inps["training"][0].shape[1] # 28  input characteristics 28 dimensions
    self.dimout = outps["training"][0].shape[1] # 20 output characteristics 20 dimensions

    self.inps, self.outps = self.load_preprocessed(inps, outps) # preprocessed
    self.num_batches = {}
    self.pointer = {}
    for key in self.inps:
      ...
      # calculate the number batch in one epoch, total 62024 sets of training data, 621 batches
  ...
  def next_batch(self, key="training"):
    ...
    for i in range(self.args.batch_size):
      ...
      n_batch = int(math.ceil((len(inp) - 2) / self.args.seq_length)) #self.args.seq_length = 100
  ...
  def train(self):
    ...
    with tf.Session() as sess:
      ...
      # TrainingStatus used to record the training process including time,loss......
